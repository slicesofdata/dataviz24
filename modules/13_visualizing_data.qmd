...
Because we passed `data = DATA` in `ggplot()`, we can reference the variables by their column names without specifying the data frame. 
...
Although you can modify the plot theme by adding it as a layer (e.g., `+ theme_minimal()`), doing so may be redundant when you do not wish to accept the default theme. Rather that add a theme layer to your plot using `theme_minimal()`, `theme_classic()`, or some other theme (e.g., `see:::theme_moderen()`), you could set the theme at the top of your code file using `theme_set()`. Setting a theme ensures that *all plots* coded in the file will adhere to that theme unless you add a layer to change it. To set the theme to `theme_minimal()`, pass that theme into `theme_set()`. 
