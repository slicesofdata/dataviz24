...
source(here::here("r", "symmspan", "sspan_agg.R"))

################################################################################
# read aggregated id data
GNG   <- readRDS(here::here("data", "gng", "gng_agg_by_id.Rds"))
names(GNG)
SSPAN <- readRDS(here::here("data", "symmspan", "sspan_agg_by_id.Rds"))
names(SSPAN)


################################################################################
# create vectors for keeping variables
gng_keep   <- c("id_subject", "wave", "target", "rt_mean", "accuracy_mean")
sspan_keep <- c("id_subject", "wave", "sspan", "totalcorrectsquares")

################################################################################
# merge by a vector
MERGE <-
  dplyr::full_join(GNG |> select(all_of(gng_keep)),
                   SSPAN |> select(all_of(sspan_keep)),
                   by = c("id_subject", "wave")
                   )

################################################################################
# MERGE |> head() |> view_html()

################################################################################
# write merged
saveRDS(MERGE, here::here("data", "merged", "gng_sspan_merged.Rds"))

message("Wrote: gng_sspan_merged.Rds")
